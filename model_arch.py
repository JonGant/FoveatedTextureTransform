...
# this script implements a Retinal Fixation V1Block VGG11 archtecture CNN with V2 inspired Foveated Texture Transform for image classification
# TODO build in maximum entropy attention module
class lp_eye_fix_VGG11FTT(nn.Module):
    def __init__(self, scale, image_size, num_classes=20):
        super(lp_eye_fix_VGG11FTT, self).__init__()
        # List of potentially different rate of growth of receptive fields
        # assuming a center fixation.
        self.scale = scale
        self.num_classes = num_classes
    ...
    # function that loads receptive fields:
    def load_receptive_fields(self, scale, image_size, conv_layer_size, index):
        def mask_transform(size):
            transform = transforms.Compose([transforms.Resize(size), transforms.Grayscale(1), transforms.ToTensor()])
            return transform
        mask_tf = mask_transform(conv_layer_size)
        mask_regular_tf = mask_transform(image_size)
        # find number of receptive fields for fixation point
        num_rf = len(glob.glob('Receptive_Fields/MetaWindows_clean_s' + scale + '_' + str(index) + '/*')) - 1
        d = 1.281  # a value that was fitted via psychophysical experiments assuming 26 deg of visual angle maps to 512 pixels on a screen.
        mask_total = torch.zeros(num_rf, conv_layer_size, conv_layer_size)
        alpha_matrix = torch.zeros(num_rf)
        for i in range(num_rf):
            i_str = str(i)
            # mask_str = './Receptive_Fields/MetaWindows_clean_s0.4/' + i_str + '.png'
            mask_str = 'Receptive_Fields/MetaWindows_clean_s' + scale + '_' + str(index) + '/' + i_str + '.png'
            mask_temp = mask_tf(Image.open(str(mask_str)))
            mask_total[i, :, :] = mask_temp
            mask_regular = mask_regular_tf(Image.open(str(mask_str)))
            mask_size = torch.sum(torch.sum(mask_regular > 0.5))
    ...
    def forward(self, content):
        ...
        content_f = self.encoder(content)
        noise_f = self.encoder(noise)
        target_prob_vector_per_fixation = []
        for i in range(1, 10):
            # initialize empty foveated feature vector
            foveated_f = torch.zeros(np.shape(content_f)[0], np.shape(content_f)[1], self.conv_size, self.conv_size).to(device)
            # receptive field loop
            # load receptive fields
            self.mask_total, self.alpha_matrix = self.load_receptive_fields(self.scale, self.image_size, self.conv_size, i)
            for j in range(len(self.alpha_matrix)):  # Loop over all the receptive fields (pooling regions)
                alpha_i = self.alpha_matrix[j]
                mask = self.mask_total[j, :, :]
                mask_binary = mask > 0.001
                content_f_mask = content_f[:, :, mask_binary[:, :]]  # 0 was 0th prefix before
                noise_f_mask = noise_f[:, :, mask_binary[:, :]]
                content_f_mask = content_f_mask.unsqueeze(3)
            ...
            vector = F.relu(self.fc2(vector))
            vector = self.fc3(vector)
            target_prob_vector_per_fixation.append(vector)
        target_prob_vector_per_fixation = torch.stack(target_prob_vector_per_fixation)
        average_of_fixation_target_probs = torch.mean(target_prob_vector_per_fixation, 0)

        return average_of_fixation_target_probs